...
def filter_dataset():
	...
	entity_type = {}
	data=[]
	with open('entities.json','r',encoding='utf8')as fr:

		for line in fr.readlines():
			try:
				temp=json.loads(line.strip())
				word=temp[ "entityOriginName"]
				type=temp["jsonNumber"]
				#word=temp["jsonItem"]["search"][0]["match"]["text"]
				#type=temp["jsonItem"]["search"][0]["match"]["type"]
	...
	with open('country-code.json','r',encoding = 'utf8') as fr:
		country_json = json.loads(fr.read())
		for x in country_json["countries"]:
			...
			for line in fr.readlines():
				...
				if(line_s[5].strip() in relation_list):
					...
					if(line_s[1].strip() in country_list and line_s[3].strip() in country_list):
						continue
					#去掉实体类型为"0"或"16"的样本
					if(line_s[1].strip() in entity_type.keys() and line_s[3].strip() in entity_type.keys()):
						...
						continue
					#if (line_s[3].strip() in entity_type.keys() i() ):
...
def get_na_entities():
	...
	with open('entities.json','r',encoding='utf8') as fr:
		for line in fr.readlines():
			...
			if(line["entityOriginName"].strip() not in entities_list):
				entities_list.append(line["entityOriginName"].strip())
